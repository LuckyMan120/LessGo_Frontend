        ...
        <div class="text-width">
            <div class="rate-item-title">
                ...
                <div class="rate-item-detail" v-if="rate.trip.points.length > 0">
                    Viajó a {{rate.trip.points[rate.trip.points.length - 1].json_address.city}} as {{rateType}}
                        - {{rate.rate_at | moment("DD/MM/YYYY")}}
                </div>
                <div class="rate-item-detail" v-else>
                    Viajó a {{ rate.trip.to_town }} as {{rateType}}
                        - {{rate.rate_at | moment("DD/MM/YYYY")}}
                </div>
            </div>
            ...
            <div v-if="showReply">
                ...
                <button class="btn btn-primary" @click="onReply"> Reply </button>
